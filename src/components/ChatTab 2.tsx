...
export const ChatTab: React.FC<ChatTabProps> = ({ showToast }) => {
      ...
      if (storedSessionId) {
        ...
        try {
          ...
          if (response.data.messages.length > 0) {
            ...
            if (historySessionId && historySessionId !== storedSessionId) {
              console.warn('⚠️ 기록 로드 시 세션 ID 불일치:', {
                stored: storedSessionId,
                history: historySessionId,
                action: '최신 세션 ID로 업데이트'
              });
              
              setSessionId(historySessionId);
              localStorage.setItem('chatSessionId', historySessionId);
            }
          }
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      // 세션 정보 업데이트 (동기화된 session_id 사용)
      console.log('API Response:', response.data);
      console.log('Model Info:', response.data.model_info);
      console.log('세션 동기화 상태:', { 
        original: sessionId, 
        synchronized: backendSessionId || sessionId 
      });
      
      const newSessionInfo = {
        sessionId: backendSessionId || sessionId, // 실제 백엔드 세션 ID 사용
